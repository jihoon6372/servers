...
export async function getFileContents(
  ...
  let url = `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/contents/${path}`;
  if (branch) {
    url += `?ref=${branch}`;
  }
...
export async function createOrUpdateFile(
  ...
  const url = `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/contents/${path}`;
  const body = {
    message,
...
async function createTree(
  ...
  const response = await githubRequest(
    `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/git/trees`,
    {
      method: "POST",
      body: {
        tree,
...
async function createCommit(
  ...
  const response = await githubRequest(
    `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/git/commits`,
    {
      method: "POST",
      body: {
        message,
...
async function updateReference(
  ...
  const response = await githubRequest(
    `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/git/refs/${ref}`,
    {
      method: "PATCH",
      body: {
        sha,
...
export async function pushFiles(
  ...
  const refResponse = await githubRequest(
    `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/git/refs/heads/${branch}`
  );
