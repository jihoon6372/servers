...
export async function createRepository(options: CreateRepositoryOptions) {
  const response = await githubRequest(
    `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/user/repos`,
    {
      method: "POST",
      body: options,
    }
...
export async function searchRepositories(
  ...
  const url = new URL(
    `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/search/repositories`
  );
...
export async function forkRepository(
  ...
  const url = organization
    ? `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/forks?organization=${organization}`
    : `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/forks`;
