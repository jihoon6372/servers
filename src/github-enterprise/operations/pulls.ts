import { z } from "zod";
import { githubRequest, getRepositoryInfo } from "../common/utils.js";
import {
...
export const CreatePullRequestReviewSchema = z.object({
  ...
  commit_id: z
    .string()
    .optional()
    .describe("The SHA of the commit that needs a review"),
      ...
      z.union([
        ...
        z.object({
          ...
          line: z
            .number()
            .describe(
              "The line number in the file where you want to add a review comment"
            ),
          body: z.string().describe("Text of the review comment"),
        }),
      ])
...
export async function createPullRequest(
  ...
  const response = await githubRequest(
    `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/pulls`,
    {
      method: "POST",
      body: options,
    }
...
export async function getPullRequest(
  ...
  const response = await githubRequest(
    `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/pulls/${pullNumber}`
  );
  return GitHubPullRequestSchema.parse(response);
}
...
export async function createPullRequestReview(
  ...
  pullNumber: number,
  options: Omit<
    z.infer<typeof CreatePullRequestReviewSchema>,
    "owner" | "repo" | "pull_number"
  >
): Promise<z.infer<typeof PullRequestReviewSchema>> {
  const response = await githubRequest(
    `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/pulls/${pullNumber}/reviews`,
    {
      method: "POST",
      body: options,
    }
...
export async function mergePullRequest(
  ...
  return githubRequest(
    `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/pulls/${pullNumber}/merge`,
    {
      method: "PUT",
      body: options,
    }
...
export async function getPullRequestFiles(
  ...
  const response = await githubRequest(
    `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/pulls/${pullNumber}/files`
  );
...
export async function updatePullRequestBranch(
  ...
  await githubRequest(
    `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/pulls/${pullNumber}/update-branch`,
    {
      method: "PUT",
...
export async function getPullRequestComments(
  ...
  const response = await githubRequest(
    `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/pulls/${pullNumber}/comments`
  );
...
export async function getPullRequestStatus(
  ...
  const response = await githubRequest(
    `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/commits/${sha}/status`
  );
