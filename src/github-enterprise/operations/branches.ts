...
export async function getDefaultBranchSHA(
  ...
  try {
    const response = await githubRequest(
      `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/git/refs/heads/main`
    );
    const data = GitHubReferenceSchema.parse(response);
    return data.object.sha;
  } catch (error) {
    const masterResponse = await githubRequest(
      `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/git/refs/heads/master`
    );
    if (!masterResponse) {
      throw new Error(
...
export async function createBranch(
  ...
  const response = await githubRequest(
    `${GITHUB_REPOSITORY_URL}${GITHUB_API_VERSION}/repos/${owner}/${repo}/git/refs`,
    {
      method: "POST",
      body: {
        ref: fullRef,
